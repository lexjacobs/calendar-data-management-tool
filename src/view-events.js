...
const IndividualEventBlock = Backbone.View.extend({
  ...
  renderSpinner() {
    ...
    // initialLoad set to true in index.js after database hydrated
  ...
  filterCollection(collection) {
    ...
    // special case for shading, and that there are 3 cases for shading
...
const FilterChooser = Backbone.View.extend({
  ...
  clickHandler(e) {
    ...
    // filtering based on data-filter attribute in html
    let clickTarget = $(e.target).data('filter');

    // ignore click events generated by unintended elements
    ...
    // filter by repetition pattern
    } else if (['annual', 'variable', 'banner'].includes(clickTarget)) {
      this.model.set({'filter': {repeat: clickTarget}});
    } else {
      console.error('filtering on unhandled type');
    }
  },
